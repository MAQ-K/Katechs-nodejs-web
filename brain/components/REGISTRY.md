...
| CountUp | `Common/motion/CountUp.js` | `value` prop | framer `animate` + `useInView` | Counts a NUMBER up from 0 once in view. Generalised from the page-scoped string version in `Emails/Features.js` (left alone — different page). Renders the final value immediately under reduced motion: the information must never depend on the animation. |
| Area Overview | `Services/ServiceArea/Overview.js` | `area` prop | Reveal + stagger | GENERIC section 1 for any service area. Split; media div first in DOM so RTL puts text right. Was `BusinessWebsites/Overview.js`. |
| Area Plans | `Services/ServiceArea/Plans.js` | `area` prop | Reveal + feature stagger, whileHover lift | GENERIC section 2. CSS-grid 3-up, EG/SA currency toggle, static navy highlight on the popular tier. 🔴 Prices are `"—"` placeholders in every area. |
| Area Faq | `Services/ServiceArea/Faq.js` | `area` prop | react-accessible-accordion + stagger | GENERIC section 3. Navy-tinted variant of `.app-faq-accordion`, same `::before:none` double-chevron fix. |
| Smooth Scroll (hook) | `Services/useSmoothScroll.js` | — | rAF lerp toward a wheel-driven target | Eased "sliding" wheel scroll. Drives the REAL document scroll (not a wrapper transform) so `position:fixed` chrome, the native scrollbar and find-in-page all keep working. Wheel only — keyboard/scrollbar/touch stay native. Adds `html.wsv-smooth` to kill Bootstrap`s `scroll-behavior:smooth`, which would otherwise animate every frame of the loop. Exports `scrollToY` for the SideRail. Off under reduced motion. |
