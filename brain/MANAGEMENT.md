...
| T-015 | Sub-pages — design, UX, content | `/ag-ui-build` + `/ag-content` | REVIEW | per page | Scope = `Web/Backup/ar/non-service-pages.md`, **in place in `style.scss`**, page by page (user, 2026-09-03). **Pass 2 (08-31, predates G9)** migrated 16 routes onto the navy/cyan `.ds-*` layer by re-pointing shared classes — **now stale per G9, not yet redone.** **Pass 3+4:** `/about-us`, `news/*` (index + 5 blog details), `useful-articles`, `case-studies-details` — clears the 08-31 "not yet migrated" backlog, but also built on the now-superseded navy/cyan layer. **Pass 5 (09-04): those same 4 blocks redone onto `.ds-new-*` monochrome** (per G9, decision 2026-09-01) — zero `--ds-cyan`/`--ds-shadow-card` left in any of the four wrapper blocks, all 11 routes verified 200. **Still unaddressed by any pass: `coming-soon`, `404`** (never explicitly covered) **and `offers`** (blocked on T-022). **⚠️ Pass 2's 16 routes are the next stale surface — same navy/cyan → monochrome conversion pass 5 just did, not yet applied there.** ⚠️ `rtl.css` loads AFTER `style.css` and carries flat rules for legacy classes — scope every new rule under a page wrapper (`.about-page`/`.news-grid-page`/`.news-details-page`/`.case-details-page` are the precedents) or it silently does nothing. Content debt flagged, not fixed: fake seeded comments on the blog-detail pages, English Lorem Ipsum on `case-studies-details` and `Common/CTA` — see T-020. See `brain/logs/2026-09-03.md` and `2026-09-04.md` |
