...
Before launching:
1. Check `brain/locks/` — if the target zone is already claimed, do NOT launch. Tell the user who holds it.
2. Give the agent: the task ID from `brain/MANAGEMENT.md` (create the row if missing), the approved idea file
   from `brain/ui-library/`, the relevant `brain/components/REGISTRY.md` rows, and the Lab's UI Kit recipe.

**Tell the agent it must not run git** — it writes files and reports; the user pushes manually.
